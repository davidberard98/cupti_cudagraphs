...
#define CUPTI_CALL(call)                                                       \
  [&]() -> CUptiResult {                                                       \
    ...
    if (_status_ != CUPTI_SUCCESS) {                                           \
      ...
      std::cerr << "function" << #call << " failed with error " << _errstr_    \
                << " (" << (int)_status_ << ")" << std::endl;                  \
    }                                                                          \
    return _status_;                                                           \
  }()

void CUPTIAPI bufferRequestedTrampoline(uint8_t **buffer, size_t *size,
                                        size_t *maxNumRecords) {
  std::cout << "  | bufferRequestedTrampoline stub" << std::endl;
}
void CUPTIAPI bufferCompletedTrampoline(CUcontext ctx, uint32_t streamId,
                                        uint8_t *buffer, size_t /* unused */,
                                        size_t validSize) {
  std::cout << "  | bufferCompletedTrampoline stub" << std::endl;
}
static void CUPTIAPI callback_switchboard(void * /* unused */,
...
ProfilingState::ProfilingState() {
  ...
  CUptiResult lastCuptiStatus_ = CUPTI_CALL(cuptiSubscribe(
  ...
  // NOTE: KIND_RUNTIME does NOT have the same issue.
